...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.data-mitra.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_mitra') ? 'is-invalid' : '' }}" type="text"
                    name="nama_mitra" id="nama_mitra" value="{{ old('nama_mitra', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lokasi_sampah') ? 'is-invalid' : '' }}" type="text"
                    name="lokasi_sampah" id="lokasi_sampah" value="{{ old('lokasi_sampah', '') }}">
            ...
            @can('admin-only')
            <div class="form-group">
                <label class="required" for="nama_user">{{ trans('cruds.baselineTarget.fields.nama_user') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all"
                        style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all"
                ...
                <select class="form-control select2 {{ $errors->has('nama_user') ? 'is-invalid' : '' }}"
                    name="nama_user[]" id="nama_user" required multiple>
                    @foreach($nama_users as $nama_user)
                    <option value="{{ $nama_user->id }}" {{ old('nama_user') ==  $nama_user->id ? 'selected' : '' }}>
                        {{ $nama_user->name }}
                    </option>
                    @endforeach
                </select>
            ...
            @endcan
            @cannot('admin-only')
            <input id="nama_user" type="hidden" name="nama_user[]" value={{Auth::id()}}>
            @endcannot
            <div class="form-group">
...
@section('scripts')
{{-- function hide(){
    var earrings = document.getElementById('nama_user');
