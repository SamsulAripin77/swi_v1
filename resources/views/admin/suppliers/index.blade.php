            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-Supplier">
                <thead>
                    <tr>
                        ...
                        @can('admin-only')
                        <th>
                            {{ trans('cruds.supplier.fields.id_user') }}
                        </th>
                        @endcan
                        <th>
                            &nbsp;
                ...
                <tbody>
                    ...
                    <tr data-entry-id="{{ $supplier->id }}">
                        ...
                        <td>
                            @foreach($supplier->jenis_plastiks as $key => $item)
                            <span class="badge badge-info">{{ $item->nama_plastik }}</span>
                            @endforeach
                        </td>
                        <td>

                            @foreach($supplier->sumber_sampahs as $key => $item)
                        ...
                        @can('admin-only')
                        <td>
                            @foreach($supplier->id_users as $key => $item)
...
@endcan
  ...
  let table = $('.datatable-Supplier:not(.ajaxTable)').DataTable({ buttons: dtButtons, scrollY: '70vh',scrollX: true})
