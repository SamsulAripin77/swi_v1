    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.penjualans.update", [$penjualan->id]) }}"
            ...
            <div class="form-group">
                <label for="tgl_jual">{{ trans('cruds.penjualan.fields.tgl_jual') }}</label>
                <input class="form-control date {{ $errors->has('tgl_jual') ? 'is-invalid' : '' }}" type="text"
                    name="tgl_jual" id="tgl_jual" placeholder="tanggal penjualan format: tgl/bln/thn"
                    value="{{ old('tgl_jual', $penjualan->tgl_jual) }}" required>
            ...
            <div class="form-group">
                <label for="nama_buyer_id">{{ trans('cruds.penjualan.fields.nama_buyer') }}</label>
                <select class="form-control select2 {{ $errors->has('nama_buyer') ? 'is-invalid' : '' }}"
                    name="nama_buyer_id" id="nama_buyer_id" required>
                    @foreach($nama_buyers as $id => $nama_buyer)
                    <option value="{{ $id }}"
                        {{ (old('nama_buyer_id') ? old('nama_buyer_id') : $penjualan->nama_buyer->id ?? '') == $id ? 'selected' : '' }}>
                        {{ $nama_buyer }}</option>
                    @endforeach
                </select>
                @if($errors->has('nama_buyer'))
                <div class="invalid-feedback">
                    {{ $errors->first('nama_buyer') }}
                </div>
                @endif
            ...
            <div class="form-group mt-2">
                ...
                <input class="total-berat form-control {{ $errors->has('total_berat') ? 'is-invalid' : '' }}"
                    ...
                    value="{{ old('total_berat', $penjualan->total_berat) }}" step="1" required>
            ...
            <div class="form-group">
                <label for="photo_manifes" class="required">{{ trans('cruds.penjualan.fields.photo_manifes') }}</label>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('konfirmasi') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="konfirmasi" id="konfirmasi" value="1"
                        {{ $penjualan->konfirmasi || old('konfirmasi', 0) === 1 ? 'checked' : '' }} required>
...
<script>
    ...
    if (berat.classList.contains('Olahan')){
            ...
            <div class="row">
                ...
                <div class="col-lg-9">
                    <textarea rows="2" class="form-control {{ $errors->has('deskripsi') ? 'is-invalid' : '' }}" name="deskripsi"
                    id="deskripsi" required>{{ old('deskripsi',$penjualan->deskripsi) }}</textarea>
                </div>
                </div>
                @if($errors->has('deskripsi'))
                <div class="invalid-feedback">
