...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pembelian.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pembelians.update", [$pembelian->id]) }}"
            enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="tgl_beli">{{ trans('cruds.pembelian.fields.tgl_beli') }}</label>
                <input class="form-control date {{ $errors->has('tgl_beli') ? 'is-invalid' : '' }}" type="text"
                    name="tgl_beli" id="tgl_beli" value="{{ old('tgl_beli', $pembelian->tgl_beli) }}"
                    placeholder="tanggal pembelian format: tgl/bln/thn" required>
            ...
            <div class="form-group">
                <label class="required"
                    for="nama_supplier_id">{{ trans('cruds.pembelian.fields.nama_supplier') }}</label>
                <select class="form-control {{ $errors->has('nama_supplier') ? 'is-invalid' : '' }}"
                    name="nama_supplier_id" id="nama_supplier_id" required>
                    @foreach($nama_suppliers as $id => $nama_supplier)
                    <option value="{{ $id }}"
                        {{ (old('nama_supplier_id') ? old('nama_supplier_id') : $pembelian->nama_supplier->id ?? '') == $id ? 'selected' : '' }}>
            ...
            @include('partials.jenis_plastik_edit')
            <div class="form-group">
                <label class="requried" for="total_berat">{{ trans('cruds.pembelian.fields.total_berat') }}</label>
                <input class="total-berat form-control {{ $errors->has('total_berat') ? 'is-invalid' : '' }}"
                    type="number" name="total_berat" id="total_berat" step=".1"
                    value="{{ old('total_berat', $pembelian->total_berat) }}" step="1"
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status_plastik') ? 'is-invalid' : '' }}"
                    name="status_plastik" id="status_plastik" required>
                    <option value="New supplier"
                        {{ (old('status_plastik') ? old('status_plastik') : $pembelian->status_plastik ?? '') == 'New supplier' ? 'selected' : '' }}>
                        New Supplier</option>
                    <option value="White space"
                        {{ (old('status_plastik') ? old('status_plastik') : $pembelian->status_plastik ?? '') == 'White space' ? 'selected' : '' }}>
                        White space</option>
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('konfirmasi') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="konfirmasi" id="konfirmasi" value="1"
                        {{ $pembelian->konfirmasi || old('konfirmasi', 0) === 1 ? 'checked' : '' }} required>
...
<script>
    ...
    init: function () {
      ...
      var files = {!! json_encode($pembelian->photo_manifes) !!}
          for (var i in files) {
          var file = files[i]
