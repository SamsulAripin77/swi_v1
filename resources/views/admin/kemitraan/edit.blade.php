...
<div class="card">
  ...
  <div class="card-body">
    <form method="POST" action="{{ route("admin.kemitraan.update", $kemitraan->id) }}" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="form-group">
        ...
        <input class="form-control date {{ $errors->has('tgl_beli') ? 'is-invalid' : '' }}" type="text" name="tgl_beli"
          id="tgl_beli" value="{{ old('tgl_beli', $kemitraan->tgl_beli) }}" placeholder="tanggal pembelian" required>
      ...
      <div class="form-group">
        ...
        <select class="form-control select2 {{ $errors->has('nama_mitra') ? 'is-invalid' : '' }}" name="nama_mitra"
          ...
          @foreach($nama_mitras as $id => $nama_mitra)
          <option value="{{ $id }}"
            {{ (old('nama_mitra') ? old('nama_mitra') : $kemitraan->nama_mitras->id ?? '') == $id ? 'selected' : '' }}>
            {{ $nama_mitra }}
          </option>
          @endforeach
        </select>
      ...
      <div class="form-group">
        ...
        <select class="form-control select2 {{ $errors->has('jenis_plastiks') ? 'is-invalid' : '' }}"
          ...
          <option value="{{ $id }}"
            {{ (in_array($id, old('jenis_plastiks', [])) || $kemitraan->jenis_plastiks->contains($id)) ? 'selected' : '' }}>
      ...
      <div class="form-group">
        ...
        <input class="total-berat form-control {{ $errors->has('total_berat') ? 'is-invalid' : '' }}" type="number"
          name="total_berat" id="total_berat" value="{{ old('total_berat', $kemitraan->total_berat) }}"
      ...
      <div class="form-group">
        ...
        <select class="form-control select2 {{ $errors->has('nama') ? 'is-invalid' : '' }}" name="nama[]" id="nama"
          ...
          <option value="{{ $id }}"
            {{ (in_array($id, old('nama', [])) || $kemitraan->nama_users->contains($id)) ? 'selected' : '' }}>
            {{ $nama_user }}</option>
          @endforeach
        </select>
        @if($errors->has('nama'))
      ...
      <div class="form-group">
        <div class="form-check {{ $errors->has('menyetujui') ? 'is-invalid' : '' }}">
          <input class="form-check-input" type="checkbox" name="menyetujui" id="menyetujui" value="1"
            {{ $kemitraan->menyetujui || old('menyetujui', 0) === 1 ? 'checked' : '' }} required>
...
Dropzone.options.photoDropzone = {
    ...
    init: function () {
@if(isset($kemitraan) && $kemitraan->photo)
      var files = {!! json_encode($kemitraan->photo) !!}
          for (var i in files) {
          var file = files[i]
...
<script>
  Dropzone.options.videoDropzone = {
    ...
    init: function () {
@if(isset($kemitraan) && $kemitraan->video)
      var file = {!! json_encode($kemitraan->video) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
