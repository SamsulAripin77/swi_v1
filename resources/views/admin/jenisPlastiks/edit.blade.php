...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.jenisPlastik.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.jenis-plastiks.update", [$jenisPlastik->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kategori_plastik') ? 'is-invalid' : '' }}" name="kategori_plastik_id" id="kategori_plastik_id" required>
                    @foreach($kategori_plastiks as $id => $kategori_plastik)
                        <option value="{{ $id }}" {{ (old('kategori_plastik_id') ? old('kategori_plastik_id') : $jenisPlastik->kategori_plastik->id ?? '') == $id ? 'selected' : '' }}>{{ $kategori_plastik }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_plastik') ? 'is-invalid' : '' }}" type="text" name="nama_plastik" id="nama_plastik" value="{{ old('nama_plastik', $jenisPlastik->nama_plastik) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" type="text" name="keterangan" id="keterangan" value="{{ old('keterangan', $jenisPlastik->keterangan) }}">
